...
class StatusesTest(TestCase):
    ...
    def test_label_bad_unique_create(self):
        ...
        response = self.client.post(
            self.create_url, follow=True, data={"name": "label4"}
        )
    ...
    def test_label_bad_unique_update(self):
        ...
        response = self.client.post(
            self.update_url, follow=True, data={"name": self.label1.name}
        )
        self.assertContains(response, "Task label с таким Имя уже существует.")
    ...
    def test_label_good_update(self):
        ...
        response = self.client.post(
            self.update_url, follow=True, data={"name": self.label2.name + "!"}
        )
    ...
    def test_label_bound_bad_delete(self):
        ...
        response = self.client.post(
            create_task_url,
            data={"name": "task5", "status": "2", "labels": ["1", "2"]},
            follow=True,
        )
